...
module Sanctum
  module Command
    ...
    class Base
      ...
      # TODO: Fix! This is a bit hacky, will update once vault-ruby gets
      # updated with better support for v2 api.
      ...
      # TODO: Most of this stuff should probably be done in a separate class,
      # or even back in Samctum::GetConfig
      ...
      # [{:name=>"vault-test", :prefix=>"vault-test", :path=>"vault/vault-test", :secrets_version=>"2"},{}]
      def set_secrets_version(targets, default_secrets_version)
        targets.each do |h|
          ...
          if default_secrets_version == 'auto'
            ...
            prefix = h[:prefix].lines('/').first.to_s
            ...
            if mounts_hash.dig(:data, :secret, prefix, :options).nil? # rubocop:disable all
              h[:secrets_version] = '1'
            else
      ...
      def set_transit_key(targets, default_transit_key)
        ...
        if targets.any? { |h| h.dig(:transit_key).nil? }
          raise <<~EOF
            transit_key must be specified under sanctum defaults, or on a per target bases
          EOF
        end
      ...
      # Parameter is an array of hashes: [{}, {}]
      # Returns array of hashes:
      # [{:name=>"vault-test", :prefix=>"vault-test/data", :path=>"vault/vault-test", :secrets_version=>"2"}]
      def update_prefix(targets)
        targets.each do |h|
          ...
          if path_array.count == 1 # rubocop:disable all
      ...
      def unable_to_determine_version
        warn red(
          "Unable to automatically gather info about mounts."\
          "This maybe due to vault connectivity or permissions"\
          "\nTo list info about mounts you may need to have following permissions added"\
          "\npath \"sys/internal/ui/mounts\" { capabilities = [\"read\"] }"\
          "\nAlternitivley add `secrets_version: <version>` for each target"\
          "specified in sanctum.yaml to bypass autodetect"
        )
      end
    end
  end
