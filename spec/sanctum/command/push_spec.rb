...
RSpec.describe Sanctum::Command::Push do
  ...
  context 'kv version 2 secrets backend' do
    ...
    it 'does not push to vault if there are no differences' do
      Sanctum::VaultTransit.write_to_file(vault_client, { "#{config_path}/#{options.dig(:sync).first.dig(:path)}/iad/prod/env" => { 'keytwo' => random_value_two.to_s } }, options[:sanctum][:transit_key])
